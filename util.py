...
class Printer:
  ...
  def print(self, text):
    ...
    for message in messages:
      text=message.strip()
      if text.startswith("~~~"):
        sleepTime1=float(text[3:6])
        text=text[6:]
      elif text.startswith("~~"):
        sleepTime1=int(text[2:4])
        text=text[4:]
      elif text.startswith("~"):
        sleepTime1=int(text[1])
        text=text[2:]
      if text.endswith("&&&"):
        sleepTime2=float(text[-6:-3])
        text=text[:-6]
      elif text.endswith("&&"):
        sleepTime2=int(text[-4:-2])
        text=text[:-4]
      elif text.endswith("&"):
        sleepTime2=float(text[-2])
        text=text[:-2]
      time.sleep(sleepTime1)
      self.printWidth(text)
      time.sleep(sleepTime2)
 
  def printWidth(self, text):
  ...
  def controls(self):
    ...
    To USE a spefic object, type 'use' before the name of the object you want to use. (eg.'use stick', 'use coin')
